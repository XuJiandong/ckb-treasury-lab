...
## Processing
The initiator can collect votes before the proposal cell is consumed. 

The script iterates over all cells in `cell_deps` to find a proposal type script whose ckb-blake160-hash matches `args`.
...
The time to collect vote cells is not strict; collecting them before the proposal cell is consumed is acceptable. Since the proposal cells and counting cells are controlled by the initiator, it can count the cells exactly before consuming the proposal cell.
